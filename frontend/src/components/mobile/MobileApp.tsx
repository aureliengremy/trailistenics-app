      ...
      <div className="m-list">
        {planned.map((p) => (
          <button key={p.k} className="m-row" onClick={() => prog.toggleSession(p.k)}>
            ...
            <Check on={!!prog.s.sessions[p.k]} col={p.col} variant="m" />
          </button>
        ))}
      </div>
...
function Renfo({ plan, prog }: { plan: PlanData; prog: ProgressApi }) {
  ...
  return (
    <div className="m-screen">
      ...
      {done > 0 && (
        <button className="m-link" onClick={prog.resetEx}>
  ...
  return (
    <div className="m-screen">
      ...
      <div className="m-tiles">
        <Tile n={(totalD / 1000).toFixed(1)} u=" k" l="m D+ cumulés (longues)" />
        <Tile n={String(Math.round(totalT / 60))} u=" h" l="de sorties longues" />
        <Tile n="740" u=" m" l="D+ jour de course" />
        <Tile n={String(13 - cur)} u="" l="sem. avant la course" />
      </div>
